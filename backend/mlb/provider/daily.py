...
def update_MLB_games_daily():    
    ...
    try:
        ...
        print(f"Finished updating MLB games from {start_date} to {end_date}.")
        print(f"Total new games added: {added_count}")
        return added_count
        
    except Exception as e:
        session.rollback()
        print(f"Error updating MLB games: {e}")
        raise
    finally:
        close_session(session)

if __name__ == "__main__":
    added = update_MLB_games_daily()
    print(f"{datetime.now()}: Daily MLB update complete. {added} new games added.")